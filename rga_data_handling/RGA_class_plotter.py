...
def plot_data_trace(trace, masslist=None):
    ...
    for mass in list_to_plot:
        traceplot.plot(
            trace.columns[trace.time_col], trace.columns[mass], label="%s AMU" % mass
        )
...
def show_results_profile(profile):
    ...
    mass_plot.bar(
        sp.array(masses_of_interest) - 0.4, sim_MID_col, color="red", label="simulated"
    )
    if plot_mode == "bar":
...
def show_results_trace(in_trace, gas_list=None):
    ...
    time_frame = [
        rescols[time_col][0] - time_step / 2,
        rescols[time_col][-1] + time_step / 2,
    ]
    ...
    for mass in masses_of_interest:
        try:
            mass_plot.plot(
                new_columns[time_col], new_columns[mass], label="%s AMU" % mass
            )
        except KeyError:
            mass_plot.plot(
                new_columns[time_col],
                sp.zeros(len(new_columns[time_col])),
                label="%s AMU" % mass,
            )
    ...
    for key in to_plot_H:
        pres_plot.plot(
            rescols[time_col], rescols[key]["pressure"], marker="o", label=key
        )
    for key in to_plot_NH:
        pres_plot.plot(
            rescols[time_col], rescols[key]["pressure"], marker="o", label=key
    ...
    mass_plot.legend(
        bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.0, fontsize=10, labelspacing=0
    )
    pres_plot.legend(
...
def show_calibration_trace(in_trace):
    ...
    time_step = sp.mean(sp.diff(columns[time_col]))
    time_frame = [
        calibcols[time_col][0] - time_step / 2,
        calibcols[time_col][-1] + time_step / 2,
    ]
    ...
    mass_plot.plot(
        calibcols[time_col], calibcols["residual"], label="residual", marker="o"
    )
