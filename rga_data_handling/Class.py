...
known_time_labels = [
    "time",
    "hours",
]  # labels of columns which are automatically recognized as time columns in the Trace class
...
def load_default_values(new_defaults=None):
    ...
    """
    default = {
        "trace_name": "Timetrace recording",
...
def read_molecule_definition(molecules, molecule):
    ...
    if type(molecule) == dict:
        ...
        if not set(["non-H-mass", "H-atoms", "intensity"]).issubset(
            set(mol_def.keys())
        ):
...
class Trace(RGA_base_container, RGA_fitting):
    ...
    def deconvolute(
        self, H_species, non_H_species, disregard, start_time, stop_time, step, n_iter=0
    ):
        ...
        ti_list = self.columns["index"][
            (start_time <= self.columns[self.time_col])
            * (stop_time >= self.columns[self.time_col])
        ]
        if len(ti_list) == 0:
        ...
        for ti in ti_list:
            ...
            results, sim_masses = self.fit_line(
                line, n_iter=n_iter
            )  # now from RGA_fitting
        ...
        for key in self.H_species.keys():
            ...
            if type(self.H_species[key][2]) == list:
                self.rescols[key]["ratio"] = sp.array(
                    outcols[self.H_species[key][2][0]]
                )
        ...
        if self.echo:
            ...
            print(
                "Deconvolutiuon for %s done, duration %s s."
                % (self.tag["title"], self.glob_duration)
            )
    ...
    def calibrate(
        ...
        if mol_name != "unknown":
            self.calib_tag["title"] = "Calibration of %s for %s" % (
                self.tag["title"],
                mol_name,
            )
        else:
        ...
        if type(ratio_def) in [float, int]:
            ...
            self.calibcols["ratio"] = sp.array(
                outcols[self.calib_candidates_dict["ratios"][0]]
            )
    ...
    def subtract_background(self, time_window):
        ...
        if self.filled:
            ...
            bgr_range = (self.columns[self.time_col] > time_window[0]) * (
                self.columns[self.time_col] < time_window[1]
            )
            ...
            if do_bgr:
                for mass in self.header_int:
                    self.columns[mass] = self.columns[mass] - sp.mean(
                        self.columns[mass][bgr_range]
                    )
    ...
    def export(self, name=None, write_path=None, rec=False):
        ...
        if self.deconvoluted:
            ...
            out_cand_tab = export_candidates(
                self.H_species, self.non_H_species, self.disregard
            )
    ...
    def pulse_integrate(self, pulse_start=None, pulse_stop=None, remove_bgr=True):
        ...
        if self.filled:
            ...
            if list(bgr_range).count(True) > 1:
                ...
                do_bgr = False
            int_range = (self.columns[self.time_col] > pulse_start) * (
                self.columns[self.time_col] < pulse_stop
            )
            for mass in self.header_int:
                ...
                if not remove_bgr:
                    bgr = 0
                int_val = sp.trapz(
                    tempdat[int_range] - bgr, self.columns[self.time_col][int_range]
                )
        ...
        if self.deconvoluted:
            ...
            if list(bgr_range).count(True) > 1:
                ...
                do_bgr = False
            int_range = (self.rescols[self.time_col] > pulse_start) * (
            ...
            for gas in self.H_species.keys():
                ...
                if do_bgr:
                    ...
                    bgr_r = sp.trapz(
                        temp_pres[bgr_range] * temp_rat[bgr_range],
                        self.rescols[self.time_col][bgr_range],
                    ) / sp.trapz(
                        temp_rat[bgr_range], self.rescols[self.time_col][bgr_range]
                    )
                    self.background_results[gas] = {"pressure": bgr_p, "ratio": bgr_r}
                else:
                    bgr_p = 0
                int_val_p = sp.trapz(
                    temp_pres[int_range] - bgr_p, self.rescols[self.time_col][int_range]
                )
                int_val_r = sp.trapz(
                    temp_pres[int_range] * temp_rat[int_range],
                    self.rescols[self.time_col][int_range],
                ) / sp.trapz(
                    temp_pres[int_range], self.rescols[self.time_col][int_range]
                )
                self.pulse_integrated_results[gas] = {
                    "pressure": int_val_p,
                    "ratio": int_val_r,
                }
    ...
    def join_isotopes(self, isotope_list, common):
        ...
        try:
            for isotope in in_rescols:
                r_col = (
                    r_col
                    + self.rescols[isotope]["ratio"]
                    * self.rescols[isotope]["pressure"]
                    / p_col
                )
...
def join_traces(in_trace_list, echo=False):
    ...
    for entry in tracelist:
        ...
        if echo:
            print(
                "trace starts at: %s, H molecules: %s, non-H molecules: %s"
        ...
        for gas in NHM_t.keys():
            ...
            rescols_j[gas]["pressure"] = sp.concatenate(
                (rescols_j[gas]["pressure"], rc_t[gas]["pressure"])
            )
...
def join_traces_calib(in_trace_list):
    ...
    for entry in tracelist:
        ...
        print(
            "trace starts at: %s, peaks: %s" % (entry[0], ", ".join(map(str, peaks_t)))
        )
        ...
        for peak in peaks_t:
            ...
            calibcols_j["peaks"][peak] = sp.concatenate(
                (calibcols_j["peaks"][peak], cc_t["peaks"][peak])
            )
